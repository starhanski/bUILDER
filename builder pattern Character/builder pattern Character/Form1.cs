...
namespace builder_pattern_Character
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var copychar = character.DeepCopy();
            var copyImage = CopyImage(copychar);

            MessageBox.Show($"Yor Character: \n Class: {character.Class} \n Ability Power: {character.Attack} \n Abilities: {character.Abilities}");
           
        }
        private Image GetImage(Character charac)
        {
                if (charac.Class == "Warrior")
            {
                return pictureBox4.Image = Resource1.Waririor;
            }
                  
              else if (charac.Class == "Mage")
            {
                return pictureBox4.Image = Resource1.Mage;
            }
                 
              else 
                  return pictureBox4.Image = Resource1.Archer;
        }
        private Image CopyImage(Character charac)
        {
            if (charac.Class == "Warrior")
                return pictureBox5.Image = Resource1.Waririor;
            else if (charac.Class == "Mage")
                return pictureBox5.Image = Resource1.Mage;
            else
                return pictureBox5.Image = Resource1.Archer;
        }
